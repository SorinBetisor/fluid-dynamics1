...
VkVertexInputAttributeDescription *getAttributeDescriptions() {
  ...
  VkVertexInputAttributeDescription *attributeDescriptions =
      malloc(sizeof(VkVertexInputAttributeDescription) * ATTRIBUTE_COUNT);
...
void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
                  ...
                  VkDeviceMemory *bufferMemory, App *pApp) {
  ...
  VkMemoryAllocateInfo allocInfo = {
      ...
      .memoryTypeIndex =
          findMemoryType(memRequirements.memoryTypeBits, properties, pApp),
  };

  if (vkAllocateMemory(pApp->device, &allocInfo, NULL, bufferMemory) !=
      VK_SUCCESS) {
...
void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size,
  ...
  vkQueueSubmit(pApp->transferQueue, 1, &submitInfo,
                VK_NULL_HANDLE); // Submit to transfer queue
  vkQueueWaitIdle(pApp->transferQueue);

  vkFreeCommandBuffers(pApp->device, pApp->transferCommandPool, 1,
                       &commandBuffer); // Free using transfer command pool
}
...
void createVertexBuffer(App *pApp) {
  ...
  createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
               ...
               &stagingBuffer, &stagingBufferMemory, pApp);
  ...
  createBuffer(bufferSize,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &pApp->vertexBuffer,
               &pApp->vertexBufferMemory, pApp);
...
void createIndexBuffer(App *pApp) {
  ...
  createBuffer(bufferSize,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &pApp->indexBuffer,
               &pApp->indexBufferMemory, pApp);
...
void createSwapChain(App *pApp) {
  ...
  vkGetSwapchainImagesKHR(pApp->device, pApp->swapChain, &imageCount,
...
void createSyncObjects(App *pApp) {
  ...
  for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    if (vkCreateSemaphore(pApp->device, &semaphoreInfo, NULL,
                          &pApp->imageAvailableSemaphores[i]) != VK_SUCCESS ||
        vkCreateSemaphore(pApp->device, &semaphoreInfo, NULL,
                          &pApp->renderFinishedSemaphores[i]) != VK_SUCCESS ||
        vkCreateFence(pApp->device, &fenceInfo, NULL,
                      &pApp->inFlightFences[i]) != VK_SUCCESS) {
...
void createFramebuffers(App *pApp) {
  pApp->swapchainFramebuffers =
      malloc(sizeof(VkFramebuffer) * pApp->swapChainImageCount);

  for (size_t i = 0; i < pApp->swapChainImageCount; i++) {
    VkImageView attachments[] = {pApp->swapchainImageView[i]};
...
void createCommandPool(App *pApp) {
  ...
  if (vkCreateCommandPool(pApp->device, &poolInfoTransfer, NULL,
                          &pApp->transferCommandPool) !=
      VK_SUCCESS) // Added transfer
  {
...
void createImageView(App *pApp) {
  pApp->swapchainImageView =
      malloc(sizeof(VkImageView) * pApp->swapChainImageCount);
  for (uint32_t i = 0; i < pApp->swapChainImageCount; i++) {
    ...
    if (vkCreateImageView(pApp->device, &createInfo, NULL,
                          &pApp->swapchainImageView[i]) != VK_SUCCESS) {
...
void cleanup(App *pApp) {
  destroyFluidSim(pApp->device, &pApp->fluidSim);
  ...
  vkFreeCommandBuffers(pApp->device, pApp->commandPool, MAX_FRAMES_IN_FLIGHT,
                       pApp->commandBuffers);
...
void createInstance(App *pApp) {
  ...
  SDL_Vulkan_GetInstanceExtensions(pApp->window, &SDLExtensionCount,
                                   SDLExtensions);
...
void createSurface(App *pApp) {
  if (SDL_Vulkan_CreateSurface(pApp->window, pApp->instance, &pApp->surface) !=
      SDL_TRUE) {
...
SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device,
  ...
  if (details.formatCount != 0) {
    VkSurfaceFormatKHR *formats =
        malloc(sizeof(VkSurfaceFormatKHR) * details.formatCount);
    if (!formats) {
  ...
  if (details.presentModeCount != 0) {
    VkPresentModeKHR *presentMode =
        malloc(sizeof(VkPresentModeKHR) * details.presentModeCount);
    if (!presentMode) {
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(
        device, surface, &details.presentModeCount, presentMode);
...
VkExtent2D chooseSwapExtent(VkSurfaceCapabilitiesKHR *capabilities, App *pApp) {
  if (capabilities->currentExtent.width != UINT32_MAX) {
    ...
    VkExtent2D actualExtent = {.width = (uint32_t)width,
...
uint32_t rateDeviceSuitability(VkPhysicalDevice device, VkSurfaceKHR surface) {
  ...
  // NOTE: To improve performance we could favour the queue families that have
  // both graphics and present support. We could check the returned indecies and
  // check if they are the same increase the score.
...
void createLogicalDevice(App *pApp) {
  ...
  if (indices.graphicsFamily == indices.presentationFamily &&
    ...
    // Separate queue families for graphics and presentation and optionally
    // transfer
    ...
    if (indices.graphicsFamily != indices.transferFamily &&
        indices.presentationFamily != indices.transferFamily) {
      VkDeviceQueueCreateInfo transferQueueCreateInfo = {
          .sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
          .queueFamilyIndex = indices.transferFamily,
          .queueCount = 1,
  ...
  // Retrieve presentation queue (reuse graphics queue if families are
  // identical)
  if (indices.graphicsFamily == indices.presentationFamily) {
    ...
    vkGetDeviceQueue(pApp->device, indices.presentationFamily, 0,
                     &pApp->presentationQueue);
  }
  // Added
  vkGetDeviceQueue(pApp->device, indices.transferFamily, 0,
                   &pApp->transferQueue);
}
...
void setupDebugMessenger(App *pApp) {
  ...
  if (CreateDebugUtilsMessengerEXT(pApp->instance, &createInfo, NULL,
                                   &pApp->debugMessanger) != VK_SUCCESS) {
